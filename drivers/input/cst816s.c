                            ...
                            size_t buflen)
{
  ...
  if ((priv->int_pending || last_event == 0 || last_event == 2) && buflen >= outlen)
    {
...
static int cst816s_isr_handler(int irq, void *context, void *arg)
{
...
//   uint8_t gpio_pin = (BOARD_TOUCH_INT & GPIO_PIN_MASK) >> GPIO_PIN_SHIFT;
  ...
  /* Configure GPIO interrupt to be triggered. */

  ret = irq_attach(irq, cst816s_isr_handler, priv);
